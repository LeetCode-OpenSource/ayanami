...
export const runSSREffects = <Context, Returned = any>(
  ...
  return modules.length === 0
    ...
    : race(
        from(modules).pipe(
          flatMap((m) => {
            const constructor: ConstructorOf<Ayanami<any>> = typeof m === 'function' ? m : m.module
            const defaultState = (m as any).defaultState
            let cleanup = noop
            const metas = Reflect.getMetadata(SSRSymbol, constructor.prototype) || []
            const ayanamiInstance: Ayanami<unknown> = InjectableFactory.getInstance(constructor)
            const moduleName = ayanamiInstance.scopeName
            let skipCount = metas.length - 1
            const ayanamiState = ayanamiInstance.createState().fork(defaultState)
