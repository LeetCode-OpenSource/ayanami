...
# FAQ
## Type Infer
When you are writing `Effect/SSREffect` logic, if the `Effect` emmit an `Action` which infer from itself, or some `Effects` circular dependents the others, `Type infer` in TypeScript may fail.
