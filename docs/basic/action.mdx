...
# Action
Ayanami `Module` was implemented by `State` object under the hood. `State` object is very similar with redux `Store` object. It **Dispatch** actions to `reducers` and `effects`, and receive actions from `Dispatcher` and `Effect`.
...
There are two different APIs to access `Dispatcher` which matched with `Module class` in `Component`:
